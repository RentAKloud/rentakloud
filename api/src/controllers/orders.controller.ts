...
export class OrdersController {
  ...
  async createOrder(
    ...
    if (
      couponIds.length > 0 &&
      couponIds.length != Array.from(new Set(couponIds)).length
    ) {
      return new BadRequestException(
        'A coupon code can be used only once per order',
      );
    }
    ...
    const err = await this.ordersService.checkResourceLimits(
      products,
      req.user.userId,
    );
    if (err) {
    ...
    const productsSelectedFields = items.map((item, i) => {
      ...
      return {
        ...
        prices:
          p.productType === ProductType.OnlineService
            ? p.prices.filter((plan: Plan) => plan.id === item.planId)
    ...
    const outOfStock = items
      .filter(
        (i) =>
          products.find((p) => p.id === i.productId).productType ===
        ...
        (sum, curr: OrderItem) =>
          ...
          (curr.isTrial
            ...
            : curr.product.prices[0].prices.find(
                (pr: PlanPrice) => pr.priceId === curr.priceId,
              ).amount),
        0,
      );
    ...
    if (shippingMethodId) {
      sm = await this.shippingMethodsService.shippingMethod({
        id: shippingMethodId,
      });
    }
    ...
    order.taxes = rates.map((r) => ({
      ...
      amount: (
        r.rate.toNumber() *
        (subTotal.toNumber() - subscriptionsTotal)
      ).toFixed(2),
      rate: r.rate.toNumber(),
    }));
    const taxesTotal = rates.reduce((sum, curr) => {
      return (
        sum + (subTotal.toNumber() - subscriptionsTotal) * curr.rate.toNumber()
      );
    }, 0);

    const amountTotal =
      subTotal.toNumber() + shippingTotal + taxesTotal - discounts;
    order.amount = new Prisma.Decimal(amountTotal.toFixed(2));

    const _order = await this.ordersService.createOrder(order);
    _order.amount = new Prisma.Decimal(
      _order.amount.toNumber() - subscriptionsTotal,
    );
  ...
  async availableShippingMethods(@Body() body) {
    ...
    return await Promise.all(
      zones[0].shippingMethods.map(async (sm) => {
        return {
          ...sm,
  ...
  evalShippingCost(cost: string, products: Product[]): number {
    ...
    if (cost.includes('w')) {
      w = products.reduce(
        (sum, curr) => sum + (curr.weight?.toNumber() || 0),
        0,
      );
    }
    return eval(cost);
  ...
  @UseGuards(JwtAuthGuard)
  @Post('/:id')
  updateOrderStatus(@Param('id', ParseIntPipe) id: number, @Request() req) {
