...
export const CartSummary: Component<{ showAddresses?: boolean }> = (props) => {
  const { orderStore, shippingSameAsBilling, hasPhysical } =
    useCheckoutContext();
  ...
  const taxesTotal = () =>
    orderStore.taxes.reduce((curr, next) => curr + next.amount, 0);
  ...
  return (
    <>
      <Show when={props.showAddresses}>
        ...
        <p class="mb-4">
          {orderStore.billingAddress.firstName}{" "}
          {orderStore.billingAddress.lastName}
          <br />
          {orderStore.billingAddress.address}
          <br />
          {orderStore.billingAddress.address2 ? (
            <>
              {orderStore.billingAddress.address2}
              <br />
            </>
          ...
          {orderStore.billingAddress.city}, {orderStore.billingAddress.state}
          <br />
          {orderStore.billingAddress.country}
          <br />
          {orderStore.billingAddress.zip}
          <br />
          {orderStore.billingAddress.email}
          <br />
          {orderStore.billingAddress.phone}
          <br />
        </p>

        <Show
          when={!shippingSameAsBilling()}
          fallback={
            hasPhysical() && (
              <p class="mb-4">Shipping address is same as billing address.</p>
            )
          }
        >
          <h4 class="font-bold">Shipping Address</h4>
          <p class="mb-4">
            {orderStore.shippingAddress.firstName}{" "}
            {orderStore.shippingAddress.lastName}
            <br />
            {orderStore.shippingAddress.address}
            <br />
            {orderStore.shippingAddress.address2 ? (
              <>
                {orderStore.shippingAddress.address2}
                <br />
              </>
            ...
            {orderStore.shippingAddress.city},{" "}
            {orderStore.shippingAddress.state}
            <br />
            {orderStore.shippingAddress.country}
            <br />
            {orderStore.shippingAddress.zip}
            <br />
          </p>
        </Show>
      </Show>
      ...
      <Show when={!products.loading}>
        ...
        <For each={cart.items} fallback={"Your cart is empty."}>
          {(item) => {
            ...
            const price = () => getProductPrice(product(), item.planId);
            ...
            return (
              <div class="flex justify-between">
                ...
                <div>
                  ...
                  <span>
                    {" "}
                    <Icon.Cross class="inline w-5" /> {item.quantity}
                  </span>
                </div>
              </div>
            );
