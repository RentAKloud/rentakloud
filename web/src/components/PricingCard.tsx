...
import { PlanFeature, ProductPrice } from "~/types/product";
...
const PricingCard: Component<PricingCardProps> = (props) => {
  ...
  const [configText, setConfigText] = createSignal<PlanFeature>();

  const planName = plan.planName || `Plan ${props.index + 1}`;
  const features = () => {
    if (plan.features && configText()) {
      return [configText()!, ...plan.features];
    }
  ...
  const price = () =>
    showYearly() ? ppy?.amount || ppm.amount! * 12 : ppm.amount;

  createEffect(async () => {
    if (plan.configId) {
      const { result } = await ConfigsApi.one(plan.configId);
      if (result) {
        setConfigText({
          html: `${result.cpus}vCPU / ${result.ram}GB RAM / ${result.ssd}GB Disk+`,
          text: "",
        });
      }
    }
  ...
  return (
    <div class="w-full max-w-sm p-4 bg-base-100 border border-primary rounded-lg shadow sm:p-8">
      ...
      <button
        ...
        onclick={() =>
          (submitHandler || defaultSubmitAction)(
            (showYearly() && ppy?.priceId) || ppm.priceId,
          )
        }
        disabled={
          (showYearly() ? !ppy?.priceId : !ppm.priceId) || !plan.configId
        }
      >
        Choose plan
      </button>
