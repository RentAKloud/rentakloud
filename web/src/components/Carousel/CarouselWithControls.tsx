...
const CarouselWithControls: Component<CarouselProps> = (props) => {
  ...
  const isLast = () =>
    props.items.length === 0 || currSlide() === props.items.length - 1;
  ...
  return (
    <div class="relative">
      ...
      <div>
        <button
          onclick={prev}
          class="btn btn-circle absolute top-1/2 left-1"
          disabled={isFirst() || inTransit()}
        >
          <Icon.ChevronLeft />
        </button>
        <button
          onclick={next}
          class="btn btn-circle absolute top-1/2 right-1"
          disabled={isLast() || inTransit()}
        >
          <Icon.ChevronRight />
        </button>
