...
<div class="flex gap-4 mb-4">
    <div class="flex flex-col gap-2">
        ...
        <Button
            ...
            on:click={() =>
                (prices = [
                    ...prices,
                    { ...structuredClone(price), id: prices.length + 1 },
                ])}
        >
            <CopySolid />
        </Button>
    ...
    <div class="flex flex-col gap-4 mb-4 flex-1">
        {#if isOnlineService}
            <Input placeholder="ID" type="number" bind:value={price.id} />
            <Input placeholder="Plan name" bind:value={price.planName} />

            <Select
                items={configOptions}
                bind:value={price.configId}
